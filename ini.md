...
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
| 7zipDir  | set | 2.0(0)/Kirs.6 | %SmartZipDir%\7-zip | 7-zip所在文件夹 | 无效时自动探测常见安装路径（含 7-Zip-Zstandard）并写回 |
| muiltNesting  | set | 2.11(9) | 0 | 启用多文件嵌套解压 | 单文件嵌套总是会被解压
| partSkip  | set | 2.21(14)/Kirs.3–Kirs.4 | 1 | 兼容保留键（不再控制跳卷） | 任一卷从首卷开始；同组多选只解压一次；不再静默跳过非首卷
| test  | set | 2.20(13)/Kirs.4 | 0 | 启用可选完整性测试；`test=0` 时普通任务可不预先测试，但在任何配置的源包移入回收站之前 SmartZip 仍会强制完整性测试 | 警告态会关闭本次源包处理并保留源包
...
| delSource  | set | 2.0(0)/Kirs.4 | 0 | 仅干净成功时将顶层源包移入回收站（非永久删除） | 警告、失败、取消及所有分卷均保留；嵌套源包使用独立安全规则
| delWhenHasPass  | set | 2.0(0)/Kirs.4 | 0 | 仅密码解析且干净成功时将顶层源包移入回收站（非永久删除） | 此项不必启用`delSource`；警告或失败时保留源包
...
| timingLog  | set | Kirs.6 | 0 | 记录探测、测试、解压耗时 | 仅写入固定操作名和毫秒数；SFXV 临时合并耗时包含在对应操作内；默认关闭
...
| 1,2,3...  | ext | 2.0(0)/Kirs.7 | `zip`... | 源后缀名`=` `值`表示其为压缩包 | 会在打开或解压时判断
...
- 可信输出：只有 `outputState=usable` 才提交到正常目标。失败输出验证后进入 `_解压不完整_`；若隔离失败为 `quarantine_failed`，诊断会报告仍保留的临时目录。
- 退出码 `1`：只有同一 7-Zip 进程含明确 warning 证据时才视为警告，不能由后续测试进程代为证明。
- 加密 CRC：有加密证据时可提供一次密码重试；普通 CRC 不重试密码。重试未解决仍保持“密码错误或数据损坏”的歧义诊断。
- 批量模式：始终非交互，不弹出密码输入；需要密码的项目进入汇总并继续后续任务。
- 设置准确性：`test` 是可选完整性测试，但任何配置的源包移入回收站前仍强制完整测试；分卷任一卷从首卷开始、同组只处理一次；源包处理是**移入回收站**而非永久删除。
- 生产构建不含测试钩子；集成测试仅在隔离 TEMP 副本注入。
- Kirs.4 发布标签为 `v3.6-kirs.4`，是新的发布线，**不替代** Kirs.1 / Kirs.2 / Kirs.3 历史标签或 Release。
